        ...
        <div className="space-y-3">
            ...
            <div className="flex flex-wrap items-center justify-between gap-2 p-2.5 bg-slate-50 dark:bg-gray-800/80 rounded-xl border border-slate-200 dark:border-gray-700">
                <div className="flex flex-wrap items-center gap-1.5 text-xs font-semibold">
                    <span className="text-gray-500 dark:text-gray-400 mr-1 text-[11px] uppercase tracking-wider font-bold">Ageing Focus:</span>
                    ...
                    >
                        ...
                        <span className={`px-1.5 py-0.2 rounded-full text-[10px] font-extrabold ${tableAgeingFilter === 'over90' ? 'bg-white text-red-700' : 'bg-red-200 text-red-800 dark:bg-red-900 dark:text-red-200'}`}>
                    ...
                    >
                        ...
                        <span className={`px-1.5 py-0.2 rounded-full text-[10px] font-extrabold ${tableAgeingFilter === 'over135' ? 'bg-white text-rose-800' : 'bg-rose-200 text-rose-900 dark:bg-rose-900 dark:text-rose-200'}`}>
            ...
            <div className="w-full rounded-xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 overflow-x-auto shadow-xs">
                <table className="w-full text-left border-collapse text-xs min-w-[900px]">
                    <thead className="bg-gray-50 dark:bg-gray-800/90 text-gray-600 dark:text-gray-300 text-[11px] uppercase tracking-wider border-b border-gray-200 dark:border-gray-700">
                        <tr>
                            {isAdmin && (
                        ...
                        {displayedData.length === 0 ? (
                            <tr>
                                <td colSpan={isAdmin ? 11 : 10} className="px-4 py-8 text-center text-gray-500 dark:text-gray-400">
                                    ...
                                    <p className="text-[11px] text-gray-400 mt-0.5">Try selecting "All" in the Ageing Focus filter bar above.</p>
                                </td>
                            </tr>
                        ) : (
                                ...
                                <tr key={item.id} className={`${rowClass} ${isRowSelected ? 'bg-blue-50/40 dark:bg-blue-950/20' : ''} ${hasOver90Dues ? 'hover:bg-red-50/30 dark:hover:bg-red-950/20' : 'hover:bg-gray-50/80 dark:hover:bg-gray-800/60'} transition-colors`}>
                                    ...
                                    <td className="px-3 py-2">
                                        <div className="flex items-center gap-1.5 flex-wrap">
                                            ...
                                            {activePdcs.length > 0 && (
                                                <button
                                                    ...
                                                    className="inline-flex items-center gap-0.5 px-1.5 py-0.2 rounded text-[10px] font-bold bg-emerald-100 text-emerald-800 dark:bg-emerald-900/50 dark:text-emerald-300 hover:bg-emerald-200 transition-colors flex-shrink-0"
                                        ...
                                        <div className="flex flex-wrap items-center gap-x-2 gap-y-0.5 mt-0.5 text-[11px] text-gray-500 dark:text-gray-400">
                                            {item.contactNumber ? (
                                                <a href={`tel:${item.contactNumber}`} className="hover:text-green-600 transition-colors font-medium">
                                                    📞 {item.contactNumber}
                                                </a>
                                            ) : null}
                                            {item.contactPerson && (
                                                <span className="text-gray-600 dark:text-gray-300 font-medium">
                                                    {item.contactPerson} {item.contactPost ? <span className="text-gray-400 text-[10px]">({item.contactPost})</span> : null}
                                                </span>
                                            )}
                                            {item.additionalContacts && item.additionalContacts.length > 0 && (
                                                <span 
                                                    className="inline-flex items-center gap-0.5 px-1.5 py-0.2 rounded text-[10px] font-semibold bg-blue-50 text-blue-700 dark:bg-blue-950/60 dark:text-blue-300 border border-blue-200 dark:border-blue-800 cursor-help"
                                            ...
                                            {item.forecastAmount !== undefined && item.forecastAmount > 0 && (
                                                <span className="inline-flex items-center gap-0.5 px-1.5 py-0.2 rounded text-[10px] font-bold bg-emerald-100 text-emerald-800 dark:bg-emerald-900/60 dark:text-emerald-300 border border-emerald-200 dark:border-emerald-800" title={`Expected Cash Forecast: ₹${item.forecastAmount.toLocaleString('en-IN')}`}>
                                                    🎯 ₹{formatCurrency(item.forecastAmount)}
                                                </span>
                                            )}
                                            {isFuture && item.status !== FollowUpStatus.Completed && (
                                                <span className="inline-flex items-center px-1.5 py-0.2 rounded text-[10px] font-medium bg-green-100 text-green-800 dark:bg-green-900/40 dark:text-green-300">
                                    ...
                                    <td className="px-2.5 py-2 whitespace-nowrap text-xs font-medium text-gray-700 dark:text-gray-300">
                                        {isAdmin && onReassignCrm ? (
                                            <select
                                                ...
                                                className="px-1.5 py-0.5 text-[11px] border rounded bg-white dark:bg-gray-800 border-gray-300 dark:border-gray-700 font-semibold text-gray-800 dark:text-gray-200 focus:ring-2 focus:ring-green-500 max-w-[105px]"
                                                title="Reassign CRM"
                                            >
                                    ...
                                    <td className="px-2.5 py-2 whitespace-nowrap text-right">
                                       <div className="flex justify-end items-center space-x-1">
                                            ...
                                            <button
                                                onClick={() => onFollowUp(item)}
                                                className="px-2.5 py-1 text-[11px] font-bold rounded transition-colors bg-green-600 text-white hover:bg-green-700 shadow-2xs"
                                            >
                                                Follow Up
                                            </button>
                                       </div>
