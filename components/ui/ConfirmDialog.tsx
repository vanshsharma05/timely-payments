...
export const ConfirmDialog = ({ open, title, children, confirmLabel, tone = 'danger', busy = false, onConfirm, onCancel }: ConfirmDialogProps) => {
    ...
    return (
        <div className="fixed inset-0 z-[60] bg-black/60 backdrop-blur-xs flex items-center justify-center p-4" onMouseDown={(e) => { if (e.target === e.currentTarget && !busy) onCancel(); }}>
            <div role="alertdialog" aria-modal="true" aria-labelledby="confirm-title" className="bg-card rounded-[16px] shadow-e2 w-full max-w-md p-6 animate-in fade-in zoom-in-95 duration-150">
                ...
                <div className="flex items-center justify-end gap-2 mt-6 max-md:[&>button]:flex-1 max-md:[&>button]:min-h-[44px]">
                    <button ref={cancelRef} type="button" onClick={onCancel} disabled={busy} className="inline-flex items-center justify-center h-9 px-4 rounded-full text-[13px] font-semibold bg-card border border-separator-strong text-label-2 hover:bg-hover hover:text-label disabled:opacity-40">
                        Cancel
                    </button>
