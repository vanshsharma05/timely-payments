...
| ~~T65~~ | **FIXED 2026-09-18 (local)**: `chequeSummary()` returns the empty summary for a null user — the rule `scopeTo()` applies to the book; `tests/signOut.dom.test.tsx` mounts the whole app with the server mocked, presses Sign out with the book and cheques loaded, and expects the login screen with no render error. Was: **Sign out blanks the page** (pre-existing since the cheques batch, 2026-09-17; identical on `f193814` and on the Phase 8 build): after `signOut()` the render with `currentUser = null` still has the cheques in state, and `chequeSummary()` (`todayPdcMetrics` before Phase 8) calls `isResponsibleFor(currentUser!, …)` → `Cannot read properties of null (reading 'id')`. The session *is* signed out; a reload shows the login form. Fix: guard the memo (`currentUser ? chequeSummary(…) : empty`) or early-return `[]` in `chequeSummary` for a null user — and a test that renders the sign-out render. One line; not applied in the deploy task (no production-code change) | `services/metrics.ts` `chequeSummary`, `App.tsx` `todayPdcMetrics` | **P2** | HIGH |
| T66 | A dead session (every save answered 401) reads like any refused save — kept in the tab, retried, *Retry now* — and nothing says "sign in again"; a reload then loses the unsaved changes. Subscribe to the auth state (SIGNED_OUT / refresh failure) and say so, keeping the changes visible | `hooks/usePersistence.ts`, `hooks/useSession.ts` | P3 | HIGH |
| T67 | A failed first load (the customers read fails) leaves the skeleton with the banner "Could not load data…" and no way to try again short of a reload | `hooks/useSession.ts`, `App.tsx` skeleton | P3 | HIGH |
| T68 | The add-customer dialog accepts a company name already in the book; a sync treats same names (`companyKey`) as one company — the later account takes the figures, the other is settled to zero. Production has no duplicates today. A warning in the dialog would close it | `components/CustomerEditModal.tsx` | P3 | HIGH |
| T69 | A `Total`/footer row or a blank-name row in a sheet or workbook reaches the sync review as a new customer (`Unknown Company` for a blank Excel name; the Google parser skips blanks); a wrong file settles most of the book to zero if the review is confirmed (recoverable by re-syncing). Skip obvious footer names and blank names; ask twice when a sync would settle more than, say, a third of the book | `services/googleSheetService.ts`, `services/excel.ts`, `SyncReconciliationModal` | P3 | HIGH |
| T70 | No length cap on names, contacts or notes; a 1,500-char name makes the book and Reports tables 14,000 px wide inside their scroll containers (nothing in production is over 53 chars). `maxLength` on the fields and `truncate` on the name cells | `CustomerEditModal.tsx`, `CustomerDashboardView.tsx`, `ReportsView.tsx` | P4 | HIGH |
| T71 | `/api/gemini-report` checks sign-in only; the UI offers the AI summary on `canExportData`. A signed-in Collector or Viewer could call the route directly and spend Gemini credit. Mirror the right server-side | `api/gemini-report.ts` | P4 | HIGH |
