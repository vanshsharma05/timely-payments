...
| D2 | **Follow-up status is both stored and derived.** `processStatuses` writes `status` from the date; `getFollowUpCategory` re-derives it at read time; the book's Status filter and `StatusBadge` read the stored value; `FollowUpModal.handleSave` computes it a third time inline. | `googleSheetService.ts`, `types.ts`, `FollowUpModal.tsx` | stale statuses on rows nobody touched since midnight; three copies of one rule |
