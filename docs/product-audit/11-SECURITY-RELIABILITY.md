...
| Manager pressing it | half-reset + duplicates (RLS refused their deletes) | **cannot**: the section is not rendered for a Manager, the handler refuses, and `reset_book()` raises `Only an Admin can reset the book` (owner's decision, Q8, 2026-09-17) |
