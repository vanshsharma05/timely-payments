...
NEXT TASK: Owner applies `supabase/reset.sql` in the SQL editor, then approves the deploy of this batch (17-RELEASE-CHECKLIST.md). Q8 is decided and applied (Admin only; owners/collectors kept; history kept). Q15 (conflict policy) still open; then R1-B. See RECOMMENDED NEXT SESSION START.
