...
CURRENT PHASE: Phase 4 (UX) — batch 3 (the cheque register + dialog) **committed locally, NOT deployed**. Batches 2–6, the CRM UX batch and the Manager/Reports batch live (`dpl_8MdpwitnETxviwA9XWRvupPzg7th`).
CURRENT SUBPHASE: —
...
NEXT TASK: Owner reviews the cheque batch (`npm run dev`, PDC cheques tab) and answers the one question in 06 (batch 3: a "deposited" state?), then approves its deploy (no SQL step). Then Phase 4 batch 4 (the Data source tab, U14) or R1-B after Q15. See RECOMMENDED NEXT SESSION START.
...
- Unit/integration tests: NONE at baseline. **Since 2026-09-17: Vitest, 249 tests, `npm run test:run`** (incl. 24 SQL tests in PGlite). Browser suites in `scripts/tests/` (no runner yet) and `scripts/` QA scripts.
...
CURRENT TEST STATUS: unit 249/249 (money 23, edit dialog 13, `customerRowDiff` 11, `updateCustomerColumns` 5, `useCollectionSync` 12, `syncFlows` 14, `derivedStatus` 16, `statusContract` 16, `resetPlan` 10, `resetSql` 24, `resetConfirmModal` 6, `saveFailures` 13, `refreshMerge` 5, `retryIdempotency` 7, `crmWorkflow` 21, `searchScope` 9, `managerReports` 17, `chequeRegister` 27). Live-site probes unchanged since the Manager/Reports deploy. Local visual QA of the cheque workflow at 1366×768, 1440×900, 1024×768 and 390×844 (every write aborted): 0 findings.
...
CURRENT ENGINEERING WORK: the cheque batch is **committed locally on `restore-and-fix`, not pushed, not deployed**. DEPLOYMENT STATE: production = `dpl_8MdpwitnETxviwA9XWRvupPzg7th` = commit `4ad33cd` (JS bundle byte-identical; CSS +12 unused rules, see 16); `origin/restore-and-fix` = `origin/main` = `3f1f755` (docs). Previous deployment `dpl_HeLAK1WcbkKNHo9N8rsunT98YPq5` (`6a33c29`) remains promotable. Post-deploy: smoke 12/12, both probes identical to the records, manager workflow QA at 1366×768 on the live site 0 findings, production unchanged. Previous deployment `dpl_GfxvXMy7gxHCRTbHSBLJeAkxQn1o` (`7e1136a`) remains promotable. Post-deploy: smoke 12/12, both probes identical to the records, CRM workflow QA at 1366×768 on the live site 0 findings, production unchanged.
...
CHANGES NOT YET VERIFIED: the cheque batch is verified by 27 unit tests (+ the two dialog suites re-pointed at the picker), typecheck, build and local QA at four sizes, but not deployed, not seen by the owner, not used by the team. The smoke script's cheque check counts rows on `#pdc` ("150 rows") — the register now pages 50, so expect "50 rows" there after the deploy; the probes touch the book only.
...
3. If the owner approved the cheque batch: deploy per 17-RELEASE-CHECKLIST.md (no SQL step), then the smoke test (expect the PDC check to say 50 rows) and both probes (A = SHREE ATAM UDYOG, B = BHAVYA PRINT-O-FLEX, C = 3 BROTHERS ( THUKRAL HOSIERY ); the status probe takes C then A).
...
5. Update this file.
LAST UPDATED: 2026-09-17 (cheque register + dialog batch, seventeenth).
