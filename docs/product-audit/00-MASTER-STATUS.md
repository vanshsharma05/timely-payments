...
CURRENT PHASE: Phase 4 (UX) — batch 2 + 2b (the Manager's Today + Reports, the Live-stock search fix, T51) **DEPLOYED 2026-09-17 22:21 IST**. Batches 2–6, the CRM UX batch and the Manager/Reports batch live.
CURRENT SUBPHASE: —
...
NEXT TASK: Phase 4 batch 3 (the Data source tab, U14) or R1-B after Q15 — the owner's order. See RECOMMENDED NEXT SESSION START.
...
CURRENT ENGINEERING WORK: nothing in flight. DEPLOYMENT STATE (2026-09-17 22:35 IST): production = `dpl_8MdpwitnETxviwA9XWRvupPzg7th` = commit `4ad33cd` (JS bundle byte-identical; CSS +12 unused rules, see 16); `origin/restore-and-fix` = `origin/main` = `4ad33cd` (+ this docs commit). Previous deployment `dpl_HeLAK1WcbkKNHo9N8rsunT98YPq5` (`6a33c29`) remains promotable. Post-deploy: smoke 12/12, both probes identical to the records, manager workflow QA at 1366×768 on the live site 0 findings, production unchanged. Previous deployment `dpl_GfxvXMy7gxHCRTbHSBLJeAkxQn1o` (`7e1136a`) remains promotable. Post-deploy: smoke 12/12, both probes identical to the records, CRM workflow QA at 1366×768 on the live site 0 findings, production unchanged.
...
CHANGES NOT YET VERIFIED: none by tooling — the Manager/Reports batch is live and verified by 26 unit tests, byte comparison of the deployed JS bundle, the read-only smoke test, both interception probes and the workflow QA on the live site; not yet used by a manager on the new build.
...
3. Confirm `npx vercel ls --prod` still shows `dpl_8MdpwitnETxviwA9XWRvupPzg7th`. The probes take the three account names as arguments (A = SHREE ATAM UDYOG, B = BHAVYA PRINT-O-FLEX, C = 3 BROTHERS ( THUKRAL HOSIERY )); the status probe takes C then A.
...
5. Update this file.
LAST UPDATED: 2026-09-17 (Manager/Reports batch deployed and verified, sixteenth).
