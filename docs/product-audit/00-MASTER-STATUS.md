...
CURRENT PHASE: Phase 4 (UX) — batch 4 (the Data source page) **DEPLOYED 2026-09-18 02:29 IST**. Batches 2–6 and the CRM UX, Manager/Reports, cheque and Data source batches live.
CURRENT SUBPHASE: —
...
NEXT TASK: R1-B after Q15, or the remaining Setup tabs (Team & access, Message templates, Alerts) — the owner's order. See RECOMMENDED NEXT SESSION START.
...
- Unit/integration tests: NONE at baseline. **Since 2026-09-17: Vitest, 263 tests, `npm run test:run`** (incl. 24 SQL tests in PGlite). Browser suites in `scripts/tests/` (no runner yet) and `scripts/` QA scripts.
...
CURRENT TEST STATUS: unit 263/263 (money 23, edit dialog 13, `customerRowDiff` 11, `updateCustomerColumns` 5, `useCollectionSync` 12, `syncFlows` 14, `derivedStatus` 16, `statusContract` 16, `resetPlan` 10, `resetSql` 24, `resetConfirmModal` 6, `saveFailures` 13, `refreshMerge` 5, `retryIdempotency` 7, `crmWorkflow` 21, `searchScope` 9, `managerReports` 17, `chequeRegister` 27, `dataSource` 14). Live-site probes unchanged since the cheque deploy; the write-payload probe against the local build with the new Data source page identical to the records. Local visual QA of the Data source page at 1366×768, 1440×900, 1024×768 and 390×844 (every write aborted, the reset never confirmed): 0 findings.
...
CHANGES NOT YET VERIFIED: none by tooling — the Data source batch is live and verified by 14 unit tests, byte comparison of the deployed script, the read-only smoke test, both interception probes and the page QA on the live site; not yet used by a manager on the new build.
...
3. Confirm `npx vercel ls --prod` still shows `dpl_FKcvzMKdrfKVgZ1d3qL4zmAPoNCT`. The smoke script looks for "Fresh start — recovery only" on the Data source page; the probes take the three account names (A = SHREE ATAM UDYOG, B = BHAVYA PRINT-O-FLEX, C = 3 BROTHERS ( THUKRAL HOSIERY ); the status probe takes C then A). Never confirm the fresh start.
4. Phase 4 batch 4: the Data source tab (U14); or R1-B after Q15 — the owner's order. Follow-ups recorded as T50 (collector drill-down), T52 (the follow-up dialog's cheque fold), T53 (the remaining `confirm()`s).
5. Update this file.
LAST UPDATED: 2026-09-18 (Data source batch deployed and verified, twentieth).
