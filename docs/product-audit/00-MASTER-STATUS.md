...
CURRENT PHASE: Phase 4 (UX) — batch 4 (the Data source page) **committed locally, NOT deployed**. Batches 2–6, the CRM UX, Manager/Reports and cheque batches live (`dpl_Ev7XczkMQHJCBsKXRnAKMNKzXkDM`).
CURRENT SUBPHASE: —
...
NEXT TASK: Owner reviews the Data source page (`npm run dev`, Settings → Data source) and approves its deploy (no SQL step). Then R1-B after Q15, or the remaining Setup tabs. See RECOMMENDED NEXT SESSION START.
...
CHANGES NOT YET VERIFIED: the Data source batch is verified by 14 unit tests, typecheck, build, local QA at four sizes and the write-payload probe against the local build, but not deployed, not seen by the owner. The probe's sync scenario is unchanged ("Sync balances" → "Update balances N rows").
...
3. If the owner approved the Data source batch: deploy per 17-RELEASE-CHECKLIST.md (no SQL step), then the smoke test (PDC check says 50 rows) and both probes (A = SHREE ATAM UDYOG, B = BHAVYA PRINT-O-FLEX, C = 3 BROTHERS ( THUKRAL HOSIERY ); the status probe takes C then A). Never confirm the fresh start.
...
5. Update this file.
LAST UPDATED: 2026-09-18 (Data source batch, nineteenth).
