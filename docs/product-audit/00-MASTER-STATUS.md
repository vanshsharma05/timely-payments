...
CURRENT PHASE: Engineering batch 5 (SEC3 — the fresh start as one database transaction with a snapshot, Admin only) **DEPLOYED 2026-09-17 14:45 IST**; `supabase/reset.sql` applied to the project first. Batches 2–5 live. Phases 0–3 done; Phase 4 (UX) not started; R1-B waits on Q15.
CURRENT SUBPHASE: —
LAST COMPLETED TASK: Fresh-start session 2026-09-17 (ninth; Q8 decisions applied the same day — Admin only in UI, handler, function and backup policy; owners/collectors and history kept): reset re-traced (11 §3.1) and rebuilt — `supabase/reset.sql` (`book_backups`, `reset_book()` one transaction with snapshot + counts + phrase + stale-plan refusal, `restore_book_backup()` Admin-only and itself reversible), `services/reset.ts` (plan via the sync's own merge: ids kept, money only, unlisted settled), `components/ResetConfirmModal.tsx` (counts, what will not happen, backup download + checkbox, typed RESET), `App.tsx` handler (sheet first → plan → dialog → one RPC → reload from server). 37 new tests incl. 21 running the real SQL in PGlite (146 total). Local flow probe against the real book with every write aborted: 0 requests before confirmation, one `rpc/reset_book` on confirm. Production untouched.
CURRENT TASK: None in progress.
NEXT TASK: **Finish the post-deploy verification** — the browser smoke test and both interception probes are blocked because the Admin login in `.deploy.local` no longer signs in (auth record changed 13:21 IST). Owner supplies a valid Admin login (update `ADMIN_PASSWORD` in `.deploy.local`, or set `TIMELY_EMAIL`/`TIMELY_PASSWORD`), then re-run `prod-smoke` + `write-payload-probe` + `status-contract-probe` (all read-only). Then Q15 → R1-B.
...
- Unit/integration tests: NONE at baseline. **Since 2026-09-17: Vitest, 149 tests, `npm run test:run`** (incl. 24 SQL tests in PGlite). Browser suites in `scripts/tests/` (no runner yet) and `scripts/` QA scripts.
...
CURRENT TEST STATUS: unit 149/149 (money 23, edit dialog 13, `customerRowDiff` 11, `updateCustomerColumns` 5, `useCollectionSync` 12, `syncFlows` 14, `derivedStatus` 16, `statusContract` 15, `resetPlan` 10, `resetSql` 24, `resetConfirmModal` 6). Probes on the live site (eighth session) unchanged. Fresh-start flow probe on the local build against the real book, every write aborted: dialog counts 144 cheques · 709 with follow-up work · 684 listed · 1 to settle · 3,342 at nil · 0 new · 4,027 on file; 0 mutating requests before confirmation; confirm → exactly one `POST /rest/v1/rpc/reset_book` (aborted); production re-read unchanged (newest write 2026-09-17T05:15Z). No staging (Q6); the SQL is exercised only in PGlite.
...
CURRENT ENGINEERING WORK: nothing in flight. DEPLOYMENT STATE (2026-09-17 14:50 IST): production = `dpl_7mKQWQGhHHNKGnLcDe7Y13EgvHKq` = commit `3fbe7f7` (bundles byte-identical); `origin/restore-and-fix` = `origin/main` = `3fbe7f7`; `supabase/reset.sql` installed (both functions, Admin check, grants, RLS verified read-only; 0 backups — no reset has ever run). Previous deployment `dpl_8NB7JLzJoSuCKsecBwz7WiycZBgJ` (`de2d1c5`) remains promotable. Post-deploy browser verification PENDING a valid Admin login (see NEXT TASK).
...
CHANGES NOT YET VERIFIED: the fresh-start batch is live and verified by 40 unit tests, byte comparison of the deployed bundles and read-only catalog checks of the installed SQL; the browser smoke test and interception probes against the live site are pending a valid Admin login; nothing has been exercised by a human, and the live reset must never be pressed "to test it".
...
REGRESSION RISKS (fresh start): behaviour changes by design — accounts the sheet no longer lists are settled instead of deleted; owners, collectors, contacts and threads are kept (they were wiped); the reset refuses if the book changed between reading the sheet and confirming ("reload and try again"). A Manager no longer sees the section at all (Q8: Admin only). PGlite is Postgres 18; the project runs Supabase's Postgres (15/17) — the SQL uses nothing version-specific, but the first run on the real project should be watched. Standing: any change to `App.tsx`, `types.ts`, `googleSheetService.ts`, `useSupabaseSync.ts`, `repository.ts`, the dialogs or `supabase/*.sql` without the unit layer.
RECOMMENDED NEXT SESSION START:
1. Read this file, then 11-SECURITY-RELIABILITY.md (§3.0 the reset as it is now; §2.2 the concurrency brief) and 19 Q15. Run `npm run test:run` (expect 149/149) and `npm run typecheck`.
2. With a working Admin login: run the read-only smoke test and both probes (`PROBE_BASE=https://timely-payment.vercel.app`) against the live site and record the results in 16. Do not press the live reset to "test" it; the SQL tests are the test.
3. Q8 is decided and applied (Admin only; owners/collectors kept; history kept) — nothing to do.
...
6. Update this file.
LAST UPDATED: 2026-09-17 (fresh-start deploy, tenth; browser verification pending).
