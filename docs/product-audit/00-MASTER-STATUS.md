...
CURRENT PHASE: Engineering batch 4 done (status contract final — no follow-up action stores a derived status; R1-B designed, not implemented). Phases 0–3 done; Phase 4 (UX) and the remaining fix batches (R1-B after the owner answers Q15, then the reset) not started.
CURRENT SUBPHASE: —
LAST COMPLETED TASK: Status-contract session 2026-09-17 (seventh): every writer of `customers.status` found and classified (11, writer table W1–W8); the three redundant derived writers removed (follow-up dialog default path, edit dialog `statusFor`, bulk date); `getFollowUpCategory` no longer falls back to the stored word; reopen pinned exactly (`Pending`). 15 new tests (109); probe with the tab clock moved to tomorrow: urgency `[is_urgent,last_follow_up_on]`, collected `status: "Completed"`, edit-dialog date `[follow_up_date]`. R1-B concurrency brief written (11 §2.2): C1 row-level `updated_at` rejected (false conflicts after every bulk event), C2 field-aware compare-and-set recommended, C3 refetch complementary, C4 field classification; owner questions in 19 Q15. **Committed locally only — not pushed, not deployed.**
CURRENT TASK: None in progress.
NEXT TASK: Owner decides (a) pushing/deploying Options A + B + the status contract together (three local commits on `restore-and-fix`), (b) Q15 — which fields get a conflict warning, who may override, whether `updated_by` is recorded. Then Session 8 implements R1-B per 11 §2.2. See RECOMMENDED NEXT SESSION START.
...
CURRENT ENGINEERING WORK: batch 4 complete (status contract), **committed locally on `restore-and-fix` on top of Options A and B, not pushed, not deployed** — the session rule overrides the standing push-live instruction. DEPLOYMENT STATE (verified 2026-09-17): the live bundle is byte-identical (md5) to a build of `066cdcd` = C1 (`a0f0a31`) + docs → **C1 deployed**; `ee61d6f` (R1-A), `2afb5d8` (R1-C) and this session's commit are **local only** — `origin/restore-and-fix` = `origin/main` = `066cdcd`. Deploy sequence when approved: push `restore-and-fix` and `restore-and-fix:main`, `git branch -f main restore-and-fix`, `npx vercel deploy --prod --yes`, then re-run both probes against the deployed build.
...
CHANGES NOT YET VERIFIED: Options A, B and the status contract are verified by 109 unit tests, typecheck, build and two interception probes, but none has been deployed or exercised by a human; the C1 fix is live since a0f0a31.
...
1. Read this file, then 11-SECURITY-RELIABILITY.md (R1 status table, "The status contract after Option B" with the writer table, and §2.2 the concurrency brief) and 19 Q15. Run `npm run test:run` (expect 109/109) and `npm run typecheck`.
2. If the owner approved the deploy: push `restore-and-fix` and `restore-and-fix:main`, `git branch -f main restore-and-fix`, `npx vercel deploy --prod --yes`; then run `node scripts/tests/write-payload-probe.cjs <out> "SHREE ATAM UDYOG" "BHAVYA PRINT-O-FLEX" "3 BROTHERS ( THUKRAL HOSIERY )"` and `node scripts/tests/status-contract-probe.cjs <out> "3 BROTHERS ( THUKRAL HOSIERY )" "SHREE ATAM UDYOG"` against a dev server on the deployed build (both abort every write) and confirm the recorded results.
3. Session 8 — **R1-B per 11 §2.2, only after Q15 is answered**: `updateCustomerColumns(id, changes, expected)` with one `eq`/`is` per changed column from Option A's baseline (never `last_follow_up_on`; `notes` auto-merged), zero rows → re-fetch by id → conflict (or "removed"); the hook reports the conflict with field, attempted value and server value; the smallest conflict experience the owner accepts (toast first). Probe the jsonb `eq` on `notes` before relying on it. Do not touch the reset, RLS, Gemini or Q11.
4. Then a focus/visibility refetch of the book following `services/liveStock.ts` (T39), with the merge rule for rows holding unsaved local edits.
5. Then SEC3 (the reset) once Q8 is answered and a staging project exists (Q6); then Phase 4 UX audit, continuing the ledger (U24, T36, T37 are on the list).
6. Update this file.
LAST UPDATED: 2026-09-17 (status-contract session, seventh).
