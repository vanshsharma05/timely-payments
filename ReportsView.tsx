...
import { Outstanding, User, UserRole, FollowUpStatus, PdcCheque, PdcStatus, CompanyProfile, DataVisibility, getFollowUpCategory } from '../types';
...
export const ReportsView = ({
    ...
    companyProfile = {
        ...
        address: 'Chemical House, GIDC Industrial Estate, Ankleshwar - 393002, Gujarat, India',
        phone: '+91 2646 220000',
        email: 'info@shorichemicals.com',
        website: 'www.shorichemicals.com',
    ...
    const boxMetrics = useMemo(() => {
        ...
        const activePendingCount = totalCount - completedCount;
    ...
    const exportToExcel = () => {
        ...
        (window as any).XLSX.utils.book_append_sheet(wb, ws, 'Ageing_Report');
                ...
                <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center gap-3">
                        ...
                        >
                            ...
                            <option value="UNASSIGNED">⚠️ Unassigned Accounts ({data.filter(d => !d.crmOwnerId || d.crmOwnerId.toUpperCase() === 'UNASSIGNED').length})</option>
                        </select>
                    </div>
                    ...
                    <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 flex-1 lg:max-w-md">
                        ...
                        <button
                            ...
                            className="flex items-center justify-center gap-1.5 px-3.5 py-2 bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-700 hover:to-teal-700 text-white rounded-lg text-xs font-bold shadow-sm transition-all whitespace-nowrap"
            ...
            <div className="bg-white dark:bg-gray-900 rounded-xl shadow-sm border border-gray-200 dark:border-gray-800 p-4 sm:p-5">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3 pt-3.5 text-center">
                    <div className="p-2.5 bg-gray-50 dark:bg-gray-800/60 rounded-lg">
                        <p className="text-[11px] text-gray-500 dark:text-gray-400 font-medium uppercase">Total Accounts</p>
                        <p className="text-base font-extrabold text-gray-900 dark:text-white mt-0.5">{boxMetrics.totalCount}</p>
                        <p className="text-[10px] text-gray-400">{formatCurrency(boxMetrics.totalAmount)}</p>
                    </div>
                    <div className="p-2.5 bg-emerald-50/60 dark:bg-emerald-950/20 border border-emerald-100 dark:border-emerald-900/40 rounded-lg">
                        <p className="text-[11px] text-emerald-800 dark:text-emerald-300 font-medium uppercase">1-45 Days (Current)</p>
                        ...
                        <p className="text-[10px] text-emerald-600 dark:text-emerald-400">{boxMetrics.ageing1_45Count} accounts</p>
                    </div>
                    <div className="p-2.5 bg-amber-50/60 dark:bg-amber-950/20 border border-amber-100 dark:border-amber-900/40 rounded-lg">
                        <p className="text-[11px] text-amber-800 dark:text-amber-300 font-medium uppercase">46-90 Days</p>
                        ...
                        <p className="text-[10px] text-amber-600 dark:text-amber-400">{boxMetrics.ageing46_90Count} accounts</p>
                    </div>
                    <div className="p-2.5 bg-orange-50/80 dark:bg-orange-950/30 border border-orange-200 dark:border-orange-900/50 rounded-lg">
                        <p className="text-[11px] text-orange-900 dark:text-orange-300 font-bold uppercase">91-135 Days</p>
                        ...
                        <p className="text-[10px] text-orange-600 dark:text-orange-400">{boxMetrics.ageing91_135Count} accounts</p>
                    </div>
                    <div className="p-2.5 bg-rose-50 dark:bg-rose-950/40 border border-rose-300 dark:border-rose-800 rounded-lg">
                        <p className="text-[11px] text-rose-900 dark:text-rose-200 font-extrabold uppercase">&gt;135 Days Critical</p>
                        <p className="text-base font-black text-rose-700 dark:text-rose-400 mt-0.5">{formatCurrency(boxMetrics.over135Amount)}</p>
                        <p className="text-[10px] text-rose-600 dark:text-rose-300 font-semibold">{boxMetrics.over135Count} accounts</p>
                    </div>
                    <div className="p-2.5 bg-red-50 dark:bg-red-950/40 border border-red-300 dark:border-red-800 rounded-lg">
                        <p className="text-[11px] text-red-900 dark:text-red-200 font-extrabold uppercase">Total &gt;90d Overdue</p>
                        <p className="text-base font-black text-red-700 dark:text-red-400 mt-0.5">{formatCurrency(boxMetrics.over90Amount)}</p>
                        <p className="text-[10px] text-red-600 dark:text-red-300 font-semibold">{boxMetrics.over90Count} accounts</p>
                    </div>
                </div>
            </div>
            ...
            <div className="bg-white dark:bg-gray-900 rounded-xl shadow-sm border border-gray-200 dark:border-gray-800 overflow-hidden">
                ...
                <div className="p-4 sm:p-5 border-b border-gray-200 dark:border-gray-800 space-y-3">
                    ...
                    <div className="flex flex-wrap items-center gap-1.5 text-xs bg-slate-50 dark:bg-gray-800/60 p-2 rounded-lg border border-slate-200 dark:border-gray-700">
                        <span className="text-gray-500 dark:text-gray-400 font-bold uppercase text-[10px] mr-1">Filter by Ageing Bucket:</span>
                        <button
                            onClick={() => setAgeingFilter('all')}
                            className={`px-2 py-0.5 rounded text-[11px] font-semibold transition-colors ${ageingFilter === 'all' ? 'bg-gray-900 text-white dark:bg-white dark:text-gray-900' : 'text-gray-600 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-700'}`}
                        >
                            All Ageing
                        </button>
                        <button
                            onClick={() => setAgeingFilter('1-45')}
                            className={`px-2 py-0.5 rounded text-[11px] font-semibold transition-colors ${ageingFilter === '1-45' ? 'bg-emerald-600 text-white' : 'text-emerald-700 dark:text-emerald-400 hover:bg-emerald-100 dark:hover:bg-emerald-950/40'}`}
                        >
                            1-45d ({boxMetrics.ageing1_45Count})
                        </button>
                        <button
                            onClick={() => setAgeingFilter('46-90')}
                            className={`px-2 py-0.5 rounded text-[11px] font-semibold transition-colors ${ageingFilter === '46-90' ? 'bg-amber-600 text-white' : 'text-amber-700 dark:text-amber-400 hover:bg-amber-100 dark:hover:bg-amber-950/40'}`}
                        >
                            46-90d ({boxMetrics.ageing46_90Count})
                        </button>
                        <button
                            onClick={() => setAgeingFilter('91-135')}
                            className={`px-2 py-0.5 rounded text-[11px] font-bold transition-colors ${ageingFilter === '91-135' ? 'bg-orange-600 text-white' : 'text-orange-700 dark:text-orange-400 hover:bg-orange-100 dark:hover:bg-orange-950/40'}`}
                        >
                            91-135d ({boxMetrics.ageing91_135Count})
                        </button>
                        <button
                            onClick={() => setAgeingFilter('over135')}
                            className={`px-2 py-0.5 rounded text-[11px] font-bold transition-colors ${ageingFilter === 'over135' ? 'bg-rose-700 text-white' : 'text-rose-700 dark:text-rose-300 hover:bg-rose-100 dark:hover:bg-rose-950/40'}`}
                        >
                            ⛔ &gt;135d ({boxMetrics.over135Count})
                        </button>
                        <button
                            onClick={() => setAgeingFilter('over90')}
                            className={`px-2 py-0.5 rounded text-[11px] font-bold transition-colors ${ageingFilter === 'over90' ? 'bg-red-600 text-white' : 'text-red-700 dark:text-red-300 hover:bg-red-100 dark:hover:bg-red-950/40'}`}
                        >
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-left border-collapse text-xs min-w-[980px]">
                        <thead className="bg-gray-50 dark:bg-gray-800/90 text-[11px] font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider border-b border-gray-200 dark:border-gray-700">
                            <tr>
                            ...
                            {filteredReportData.length === 0 ? (
                                <tr>
                                    <td colSpan={11} className="px-4 py-10 text-center text-gray-500 dark:text-gray-400">
                                        ...
                                        <p className="text-[11px] text-gray-400 mt-1">Try switching to "All" or choosing another ageing bucket.</p>
                                    </td>
                                </tr>
                            ) : (
                                        ...
                                        <tr key={item.id} className={`${rowBorder} ${hasOver90Dues ? 'hover:bg-red-50/30 dark:hover:bg-red-950/20' : 'hover:bg-gray-50/80 dark:hover:bg-gray-800/50'} transition-colors`}>
                                            <td className="px-3 py-2.5">
                                                <div className="flex items-center gap-1.5 flex-wrap">
                                                    <button
                                                        ...
                                                        className="font-bold text-gray-900 dark:text-white hover:text-green-600 dark:hover:text-green-400 text-left flex items-center gap-1.5"
                                                    >
                                                        <span>{item.company}</span>
                                                        {item.isUrgent && <FireIcon className="text-red-500 w-3.5 h-3.5 flex-shrink-0" />}
                                                    </button>
                                                    {hasOver90Dues && (
                                                        <span className="inline-flex items-center px-1.5 py-0.2 rounded text-[10px] font-extrabold bg-red-100 text-red-800 dark:bg-red-900/60 dark:text-red-200 border border-red-200 dark:border-red-800" title="Overdue > 90 Days late payment focus">
                                                            &gt;90d
                                                        </span>
                                                    )}
                                                    {activePdcs.length > 0 && (
                                                        <button
                                                            ...
                                                            className="inline-flex items-center gap-1 px-1.5 py-0.2 rounded text-[10px] font-bold bg-emerald-100 text-emerald-800 dark:bg-emerald-900/50 dark:text-emerald-300 hover:bg-emerald-200 transition-colors flex-shrink-0"
                                                ...
                                                <div className="flex flex-wrap items-center gap-2 mt-0.5 text-[11px] text-gray-500 dark:text-gray-400">
                                                    {item.contactNumber && (
                                                        <a href={`tel:${item.contactNumber}`} className="hover:text-green-600 font-medium">
                                                            📞 {item.contactNumber}
                                                        </a>
                                                    )}
                                                    {item.contactPerson && (
                                            ...
                                            <td className="px-2 py-2.5 text-right whitespace-nowrap">
                                                <BalanceAmount
                                                    ...
                                                    defaultClass={`text-xs ${a1 > 0 ? 'text-gray-700 dark:text-gray-300 font-medium' : 'text-gray-300 dark:text-gray-600'}`}
                                                />
                                            </td>
                                            ...
                                            <td className="px-2 py-2.5 text-right whitespace-nowrap">
                                                <BalanceAmount
                                                    ...
                                                    defaultClass={`text-xs ${a2 > 0 ? 'text-amber-700 dark:text-amber-300 font-medium' : 'text-gray-300 dark:text-gray-600'}`}
                                                />
                                            </td>
                                            ...
                                            <td className="px-2.5 py-2.5 text-right whitespace-nowrap bg-amber-50/50 dark:bg-amber-950/20 border-l border-amber-100 dark:border-amber-900/40">
                                                <BalanceAmount
                                                    ...
                                                    defaultClass={`text-xs ${a3 > 0 ? 'text-amber-800 dark:text-amber-300 font-bold' : 'text-gray-300 dark:text-gray-600'}`}
                                                />
                                            </td>
                                            ...
                                            <td className="px-2.5 py-2.5 text-right whitespace-nowrap bg-red-50/50 dark:bg-red-950/20 border-l border-r border-red-100 dark:border-red-900/40">
                                                <BalanceAmount
                                                    ...
                                                    defaultClass={`text-xs ${a4 > 0 ? 'text-red-700 dark:text-red-400 font-extrabold' : 'text-gray-300 dark:text-gray-600'}`}
                                                />
                                            </td>
                                            ...
                                            <td className="px-2.5 py-2.5 text-right whitespace-nowrap">
                                                <BalanceAmount
                                                    ...
                                                    defaultClass={`text-xs ${over90Total > 0 ? 'text-red-600 dark:text-red-400 font-extrabold' : 'text-gray-300 dark:text-gray-600'}`}
                                                />
                                            </td>
                                            ...
                                            <td className="px-2.5 py-2.5 text-center whitespace-nowrap">
                                                {isTodayFollowUp(item) ? (
                                                    ...
                                                    <span className="px-2 py-0.5 rounded-full text-[11px] font-bold bg-red-100 text-red-800 dark:bg-red-900/60 dark:text-red-300">
                                                        Overdue
                                                    </span>
                                            ...
                                            <td className="px-2.5 py-2.5 text-right whitespace-nowrap">
                                                <div className="flex items-center justify-end gap-1.5">
                                                    <button
                                                        onClick={() => onFollowUp(item)}
                                                        className="px-2.5 py-1 bg-green-600 hover:bg-green-700 text-white rounded text-xs font-bold shadow-xs transition-colors"
                                                    >
                                                        Update
                                                    </button>
                                                    <button
                                                        onClick={() => onWhatsApp(item)}
                                                        className="p-1 bg-emerald-500 hover:bg-emerald-600 text-white rounded text-xs font-semibold shadow-xs transition-colors"
