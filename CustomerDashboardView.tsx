...
import { Outstanding, User, UserRole, FollowUpStatus, PdcCheque, BalanceType, DEFAULT_ROLE_PERMISSIONS, PaymentRank, getCustomerPaymentRank, DataVisibility } from '../types';
import BalanceAmount from './BalanceAmount';
import StatusBadge from './StatusBadge';
import { WhatsAppIcon, ChequeIcon, UsersIcon } from './icons/Icons';
...
export const CustomerDashboardView: React.FC<CustomerDashboardViewProps> = ({
    ...
    }, [userAllowedData, searchTerm, rankFilter, selectedCrm, ageingFilter, statusFilter, balanceTypeFilter, originFilter]);
    ...
    return (
        <div className="w-full space-y-3.5">
            {/* Top Header Banner */}
            <div className="bg-gradient-to-r from-emerald-800 via-teal-800 to-slate-900 text-white px-4 py-3 sm:px-5 sm:py-3.5 rounded-xl shadow-lg flex flex-col md:flex-row justify-between items-start md:items-center gap-3">
                <div>
                    <div className="flex items-center gap-2 flex-wrap">
                        <h1 className="text-lg sm:text-xl font-extrabold tracking-tight flex items-center gap-1.5">
                            <span>🏢 Customer Directory & Master List</span>
                        </h1>
                        ...
                        {lastUpdatedTill && (
                            <span className="px-2 py-0.5 rounded-md text-[11px] font-semibold bg-emerald-500/30 text-emerald-200 border border-emerald-400/30">
                                📅 Till: {lastUpdatedTill}
                            </span>
                        )}
                    </div>
                    <div className="flex items-center gap-2.5 mt-1 text-xs text-emerald-100 flex-wrap">
                        <span>Logged in as: <strong className="text-white underline">{currentUser?.name}</strong> ({currentUser?.role})</span>
                        <span className="opacity-40">•</span>
                        <span className="flex items-center gap-1">
                            <span className="w-2 h-2 rounded-full bg-emerald-400"></span>
                            Rights: {canAddCustomer ? '➕ Add' : ''} {canEditCustomer ? '• ✏️ Edit' : ''} {canEditFollowUp ? '• 📞 Follow-up' : ''} {canManagePdc ? '• 💳 PDC' : ''} {isAdmin ? '• 👑 Admin' : ''}
                        </span>
                    </div>
                </div>
                ...
                <div className="flex items-center gap-2 flex-wrap w-full sm:w-auto">
                    ...
                    <button
                        ...
                        className={`px-3.5 py-1.5 text-xs font-extrabold rounded-lg transition-all shadow-sm flex items-center gap-1.5 ${
                            canAddCustomer
                                ? 'bg-gradient-to-r from-emerald-500 to-green-600 hover:from-emerald-600 hover:to-green-700 text-white ring-1 ring-white/30 cursor-pointer'
                                : 'bg-gray-600/60 text-gray-300 opacity-60 cursor-not-allowed'
                        }`}
                    >
                        <span>➕ Add Customer</span>
                        {!canAddCustomer && <span className="text-[10px]">🔒</span>}
                    </button>
                    ...
                    {onSyncSheet && (
                        <button
                            onClick={onSyncSheet}
                            disabled={isSyncing}
                            className="px-3 py-1.5 text-xs font-bold rounded-lg bg-white/10 hover:bg-white/20 text-white border border-white/20 transition-all flex items-center gap-1"
                    ...
                    <div className="flex bg-black/30 p-0.5 rounded-lg border border-white/10">
                        <button
                            ...
                            className={`px-2.5 py-1 text-xs font-bold rounded transition-all ${viewMode === 'table' ? 'bg-white text-gray-900 shadow-xs' : 'text-white/80 hover:text-white'}`}
                        ...
                        <button
                            ...
                            className={`px-2.5 py-1 text-xs font-bold rounded transition-all ${viewMode === 'cards' ? 'bg-white text-gray-900 shadow-xs' : 'text-white/80 hover:text-white'}`}
            ...
            {/* Quick Metrics Bar with Good vs Bad Payment Breakdown */}
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-2">
                {/* Total Receivables */}
                <div className="p-2.5 bg-white dark:bg-gray-800 rounded-lg border border-slate-200 dark:border-gray-700 shadow-2xs">
                    <div className="text-[10px] font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Total Receivables</div>
                    <div className="text-sm sm:text-base font-extrabold text-gray-900 dark:text-white">
                        {formatCurrency(metrics.debitSum)}
                    </div>
                    <div className="text-[10px] text-gray-400">{metrics.count} active accounts</div>
                </div>

                {/* Good Payment Rank */}
                <button
                    onClick={() => setRankFilter(rankFilter === 'Good' ? 'ALL' : 'Good')}
                    className={`text-left p-2.5 rounded-lg border transition-all shadow-2xs ${
                        rankFilter === 'Good'
                            ? 'bg-emerald-100/90 dark:bg-emerald-950/60 border-emerald-500 ring-2 ring-emerald-400'
                            : 'bg-emerald-50/70 dark:bg-emerald-950/20 border-emerald-200 dark:border-emerald-900/40 hover:border-emerald-400'
                    }`}
                >
                    <div className="flex items-center justify-between">
                        <span className="text-[10px] font-bold text-emerald-700 dark:text-emerald-300 uppercase tracking-wider">⭐ Good Payment</span>
                        <span className="text-[10px] font-bold px-1.5 py-0.2 bg-emerald-200 dark:bg-emerald-900/80 text-emerald-800 dark:text-emerald-200 rounded-full">{metrics.goodCount}</span>
                    </div>
                    <div className="text-sm sm:text-base font-extrabold text-emerald-800 dark:text-emerald-200">
                        {formatCurrency(metrics.goodDebitSum)}
                    </div>
                    <div className="text-[10px] text-emerald-600 dark:text-emerald-400">Timely & healthy terms</div>
                </button>
                ...
                <button
                    ...
                    className={`text-left p-2.5 rounded-lg border transition-all shadow-2xs ${
                        rankFilter === 'Bad'
                            ? 'bg-rose-100/90 dark:bg-rose-950/60 border-rose-500 ring-2 ring-rose-400'
                            : 'bg-rose-50/70 dark:bg-rose-950/20 border-rose-200 dark:border-rose-900/40 hover:border-rose-400'
                    }`}
                >
                    <div className="flex items-center justify-between">
                        <span className="text-[10px] font-bold text-rose-700 dark:text-rose-300 uppercase tracking-wider">⚠️ Bad Payment</span>
                        <span className="text-[10px] font-bold px-1.5 py-0.2 bg-rose-200 dark:bg-rose-900/80 text-rose-800 dark:text-rose-200 rounded-full">{metrics.badCount}</span>
                    </div>
                    <div className="text-sm sm:text-base font-extrabold text-rose-800 dark:text-rose-200">
                        {formatCurrency(metrics.badDebitSum)}
                    </div>
                    <div className="text-[10px] text-rose-600 dark:text-rose-400">Credit delayed / focus needed</div>
                </button>
                ...
                <button
                    ...
                    className={`text-left p-2.5 rounded-lg border transition-all shadow-2xs ${
                        ageingFilter === 'dueOver45'
                            ? 'bg-amber-100/90 dark:bg-amber-950/60 border-amber-500 ring-2 ring-amber-400'
                            : 'bg-amber-50/70 dark:bg-amber-950/20 border-amber-200 dark:border-amber-900/40 hover:border-amber-400'
                    }`}
                >
                    <div className="text-[10px] font-bold text-amber-700 dark:text-amber-300 uppercase tracking-wider">&gt;45 Days Due</div>
                    <div className="text-sm sm:text-base font-extrabold text-amber-800 dark:text-amber-200">
                        {formatCurrency(metrics.due45Sum)}
                    </div>
                    <div className="text-[10px] text-amber-600 dark:text-amber-400">Working capital overdue</div>
                </button>
                ...
                <button
                    ...
                    className={`text-left p-2.5 rounded-lg border transition-all shadow-2xs ${
                        ageingFilter === 'over90'
                            ? 'bg-red-100/90 dark:bg-red-950/60 border-red-500 ring-2 ring-red-400'
                            : 'bg-red-50/70 dark:bg-red-950/20 border-red-200 dark:border-red-900/40 hover:border-red-400'
                    }`}
                >
                    <div className="text-[10px] font-bold text-red-700 dark:text-red-300 uppercase tracking-wider">🚨 &gt;90 Days Overdue</div>
                    <div className="text-sm sm:text-base font-extrabold text-red-700 dark:text-red-300">
                        {formatCurrency(metrics.over90Sum)}
                    </div>
                    <div className="text-[10px] text-red-600 dark:text-red-400">High recovery alert</div>
                </button>

                {/* Advance / Credit Balance */}
                <div className="p-2.5 bg-blue-50/70 dark:bg-blue-950/20 rounded-lg border border-blue-200 dark:border-blue-900/40 shadow-2xs">
                    <div className="text-[10px] font-bold text-blue-700 dark:text-blue-300 uppercase tracking-wider">Advance / Cr Balance</div>
                    <div className="text-sm sm:text-base font-extrabold text-blue-700 dark:text-blue-300">
                        {formatCurrency(metrics.creditSum)}
                    </div>
                    <div className="text-[10px] text-blue-600 dark:text-blue-400">Advance with company</div>
                </div>
            </div>

            {/* Compact Filter Controls Card */}
            <div className="p-3 bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 shadow-2xs space-y-2.5">
                {/* Search, Rank, CRM, Status Filters Row */}
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-12 gap-2">
                    {/* Live Search */}
                    <div className="lg:col-span-4 relative">
                        <label className="block text-[10px] font-bold text-gray-600 dark:text-gray-400 uppercase tracking-wider mb-0.5">
                ...
                <div className="flex flex-wrap items-center justify-between gap-1.5 pt-2 border-t border-gray-100 dark:border-gray-700/60">
                    <div className="flex flex-wrap items-center gap-1">
                        <span className="text-[10px] font-bold text-gray-500 dark:text-gray-400 mr-1">Rank:</span>
                        <button
                        ...
                        <span className="text-[10px] font-bold text-gray-400 dark:text-gray-500 mx-1">|</span>
                        <span className="text-[10px] font-bold text-gray-500 dark:text-gray-400 mr-1">Ageing:</span>
                        <button
                    ...
                    <div className="flex items-center gap-1.5">
                        <select
                            ...
                            className="text-[11px] px-2 py-0.5 rounded border border-gray-300 dark:border-gray-700 bg-gray-50 dark:bg-gray-900 font-semibold text-gray-800 dark:text-gray-200"
                        >
                        ...
                        {(searchTerm || rankFilter !== 'ALL' || selectedCrm !== 'ALL' || ageingFilter !== 'all' || statusFilter !== 'ALL' || balanceTypeFilter !== 'ALL' || originFilter !== 'ALL') && (
                            <button
                                ...
                                className="text-[11px] text-red-600 dark:text-red-400 font-bold hover:underline px-1"
                            >
                                Reset
                            </button>
                        )}
            ...
            {filteredData.length === 0 ? (
                ...
                <div className="w-full max-w-full bg-white dark:bg-gray-850 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden flex flex-col">
                    {/* Top Table Control Bar with Quick Horizontal Scroll Slider & Navigation */}
                    <div className="px-3.5 py-2.5 bg-slate-50/95 dark:bg-gray-900 border-b border-gray-200 dark:border-gray-700 flex flex-wrap items-center justify-between gap-3 text-xs">
                        <div className="flex items-center gap-2 flex-wrap">
                        ...
                        {/* Top Slider & Scroll Buttons */}
                        <div className="flex items-center gap-2 flex-wrap ml-auto">
                            <div className="flex items-center gap-1 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 px-2 py-1 rounded-lg shadow-2xs">
                                <span className="text-[10px] font-bold text-gray-500 dark:text-gray-400">Slide:</span>
                                <input
                                    type="range"
                                    min="0"
                                    max="100"
                                    ...
                                    className="w-24 sm:w-36 h-1.5 bg-gray-200 dark:bg-gray-700 rounded-lg appearance-none cursor-pointer accent-emerald-600"
                                    title="Drag slider left/right to scroll table view"
                                />
                            </div>
                            ...
                            <div className="flex items-center gap-1">
                                <button
                                    ...
                                    className="px-2 py-1 text-[11px] font-bold rounded-md bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-700 text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors shadow-2xs"
                                    title="Jump to beginning (Leftmost - Customer details)"
                                >
                                    ⏮ Start
                                </button>
                                <button
                                ...
                                <button
                                    ...
                                    className="px-2 py-1 text-[11px] font-bold rounded-md bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-700 text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors shadow-2xs"
                                    title="Jump to end (Rightmost - Follow-ups & Actions)"
                                >
                                    End ⏭
                                </button>
                            </div>
                    ...
                    >
                        <table className="min-w-[1380px] w-full text-xs text-left border-collapse table-auto">
                            <thead className="bg-slate-100/95 dark:bg-gray-800/95 text-gray-700 dark:text-gray-300 font-bold uppercase tracking-wider border-b border-gray-200 dark:border-gray-700 text-[10px] sticky top-0 z-10 backdrop-blur-xs">
                                <tr>
                                    {canReassignCrm && (
                                        ...
                                        >
                                            ...
                                            <td className="px-3.5 py-2.5 min-w-[320px]">
                                                <div className="flex items-center gap-1.5 flex-wrap">
                                                    ...
                                                    {item.isNewCustomer && (
                                                        <span className="px-1.5 py-0.2 rounded text-[9px] font-extrabold bg-blue-100 text-blue-800 dark:bg-blue-900/60 dark:text-blue-200">
                                                            NEW
                                                        </span>
                                                    )}
                                                    {item.isUrgent && (
                                                        <span className="px-1.5 py-0.2 rounded text-[9px] font-extrabold bg-red-100 text-red-800 dark:bg-red-900/60 dark:text-red-200">
                                                            URGENT
                                                        </span>
                                                    )}
                                                    {activePdcs.length > 0 && (
                                                        <button
                                                            ...
                                                            className="inline-flex items-center gap-0.5 px-1.5 py-0.2 rounded text-[9px] font-bold bg-emerald-100 text-emerald-800 dark:bg-emerald-900/60 dark:text-emerald-200 hover:bg-emerald-200 transition-colors"
                                                ...
                                                <div className="flex flex-wrap items-center gap-x-2 gap-y-0.5 mt-1 text-[11px] text-gray-500 dark:text-gray-400">
                                                    ...
                                                    {item.contactNumber && (
                                                        <a href={`tel:${item.contactNumber}`} className="font-bold text-emerald-700 dark:text-emerald-400 hover:underline whitespace-nowrap">
                                                            📞 {item.contactNumber}
                                                        </a>
                                                    )}
                                                    {item.additionalContacts && item.additionalContacts.length > 0 && (
                                                        <span 
                                                            className="px-1.5 py-0.2 rounded bg-blue-50 text-blue-700 dark:bg-blue-950/60 dark:text-blue-300 text-[9px] font-bold cursor-help whitespace-nowrap"
                                            ...
                                            <td className="px-2.5 py-2.5 text-center whitespace-nowrap">
                                                <div className="flex flex-col items-center justify-center">
                                                    ...
                                                    <div className="text-[9px] text-gray-500 dark:text-gray-400 mt-0.5 font-medium">
                                            ...
                                            <td className="px-2.5 py-2.5 text-right whitespace-nowrap">
                                                <BalanceAmount
                                                    ...
                                                    defaultClass={`text-[11px] ${a1 > 0 ? 'text-gray-700 dark:text-gray-300 font-medium' : 'text-gray-300 dark:text-gray-600'}`}
                                                />
                                            </td>
                                            ...
                                            <td className="px-2.5 py-2.5 text-right whitespace-nowrap">
                                                <BalanceAmount
                                                    ...
                                                    defaultClass={`text-[11px] ${a2 > 0 ? 'text-amber-700 dark:text-amber-300 font-semibold' : 'text-gray-300 dark:text-gray-600'}`}
                                                />
                                            </td>
                                            ...
                                            <td className="px-2.5 py-2.5 text-right whitespace-nowrap bg-amber-50/40 dark:bg-amber-950/10">
                                                <BalanceAmount
                                                    ...
                                                    defaultClass={`text-[11px] ${a3 > 0 ? 'text-amber-800 dark:text-amber-300 font-bold' : 'text-gray-300 dark:text-gray-600'}`}
                                                />
                                            </td>
                                            ...
                                            <td className="px-2.5 py-2.5 text-right whitespace-nowrap bg-red-50/40 dark:bg-red-950/10">
                                                <BalanceAmount
                                                    ...
                                                    defaultClass={`text-[11px] ${a4 > 0 ? 'text-red-700 dark:text-red-400 font-extrabold' : 'text-gray-300 dark:text-gray-600'}`}
                                                />
                                            </td>
                                            ...
                                            <td className="px-2.5 py-2.5 text-right whitespace-nowrap bg-rose-50/30 dark:bg-rose-950/10">
                                                <BalanceAmount
                                                    amount={due45}
                                                    type="Dr"
                                                    defaultClass={`text-[11px] ${due45 > 0 ? 'text-rose-700 dark:text-rose-400 font-extrabold' : 'text-gray-300 dark:text-gray-600'}`}
                                                />
                                            </td>
                                            ...
                                            <td className="px-2.5 py-2.5 text-center whitespace-nowrap">
                                                <div className="flex flex-col items-center justify-center gap-0.5">
                                                    ...
                                                    <span className={`text-[10px] font-bold ${
                                                        ...
                                                        {item.followUpDate ? formatDate(item.followUpDate) : '⚠️ No date'}
                                                    </span>
                                                    {item.forecastAmount !== undefined && item.forecastAmount > 0 && (
                                                        <span className="inline-flex items-center gap-0.5 px-1.5 py-0.2 rounded text-[9px] font-bold bg-emerald-100 text-emerald-800 dark:bg-emerald-900/60 dark:text-emerald-300 border border-emerald-200 dark:border-emerald-800" title={`Committed Cash Forecast: ₹${item.forecastAmount.toLocaleString('en-IN')}`}>
                                            ...
                                            <td className="px-2.5 py-2.5 text-left whitespace-nowrap">
                                                {canReassignCrm && onReassignCrm ? (
                                                    <select
                                                        ...
                                                        className="text-[11px] px-1.5 py-0.5 rounded border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800 font-bold text-gray-800 dark:text-gray-200 cursor-pointer"
                                                    >
                                            ...
                                            <td className="px-3 py-2.5 text-right whitespace-nowrap">
                                                <div className="flex items-center justify-end space-x-1">
                                                    ...
                                                    {canDeleteCustomer && onDeleteCustomer && (
                                                        <button
                                                            onClick={() => {
                                                                if (confirm(`Are you sure you want to delete "${item.company}"?`)) {
                                                                    onDeleteCustomer(item.id);
                                                                }
                                                            }}
                                                            className="p-1 text-red-600 hover:text-red-800 hover:bg-red-50 dark:hover:bg-red-950/40 rounded transition-colors"
                    ...
                    <div className="px-3.5 py-3 bg-slate-100/90 dark:bg-gray-900 border-t border-gray-200 dark:border-gray-700 flex flex-col sm:flex-row items-center justify-between gap-3 text-xs">
                            ...
                            >
                                <div>
                                    ...
                                    <div className="flex justify-between items-start gap-2">
                                        <div>
                                            ...
                                            <div className="flex items-center gap-1.5 mt-1 flex-wrap">
                                                ...
                                                <span className="px-2 py-0.5 rounded-md text-[10px] font-bold bg-slate-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300">
                                    ...
                                    <div className="mt-2.5 p-2 bg-slate-50 dark:bg-gray-900/60 rounded-lg space-y-1 text-xs">
                                        <div className="flex justify-between items-center text-gray-700 dark:text-gray-300">
                                            ...
                                            {item.city && <span className="text-[10px] text-gray-500 font-medium">📍 {item.city}</span>}
                                        </div>
                                        {item.contactNumber && (
                                            <div className="flex items-center justify-between text-[11px]">
                                                <a href={`tel:${item.contactNumber}`} className="font-bold text-emerald-700 dark:text-emerald-400 hover:underline">
                                                    📞 {item.contactNumber}
                                                </a>
                                                <button
                                                    onClick={() => onWhatsApp(item)}
                                        ...
                                        {(item.paymentTermsDays || item.creditLimit) && (
                                            <div className="text-[10px] text-gray-500 dark:text-gray-400 pt-1 border-t border-gray-200 dark:border-gray-800">
                                    ...
                                    <div className="grid grid-cols-4 gap-1 text-center mt-2 pt-2 border-t border-gray-100 dark:border-gray-700/60 text-[10px]">
                                ...
                                <div className="mt-3 pt-2.5 border-t border-gray-100 dark:border-gray-700/60 flex items-center justify-between gap-2">
                                    <div className="text-[11px] text-gray-500 dark:text-gray-400">
                                        {item.followUpDate ? `📅 Next: ${formatDate(item.followUpDate)}` : '⚠️ No date set'}
                                    </div>
